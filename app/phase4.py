...
def validation_state(db,project_id,scope):
    ...
    for item in items:
        ...
        valid=db.execute("SELECT id,evidence_type FROM evidence_records WHERE validation_item_id=? AND status='VALID' AND (? IS NULL OR validation_result_id=?)",(item["id"],result["id"] if result else None,result["id"] if result else None)).fetchall();evidence_ids.extend(x["id"] for x in valid);have={x["evidence_type"] for x in valid};required=set(json.loads(item["required_evidence_types_json"]))
